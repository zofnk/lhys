...
public class MagicBookView extends FrameLayout {
    ...
    public void setCurrentPage(int page) {
        ...
        mPrePageContainter.doTurnReload(true, mCurrentPage);

        mCurPageContainter.setCurPageInBook(mCurrentPage);
        mCurPageContainter.doTurnReload(false, mCurrentPage);

        mNextPageContainter.setCurPageInBook(mCurrentPage + 1);
        mNextPageContainter.doTurnReload(false, mCurrentPage);
    }

    /**
     * isTurnBack=true上一页，isTurnBack=false下一页，
     *
     * @param isTurnBack
     */
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mPrePageContainter != null && mCurPageContainter != null && mNextPageContainter != null) {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mIsInterActMode) {
            ...
            if (DEBUG)
                Log.v(TAG, "mCurPageContainter.getCurPageInBook() = " + mCurPageContainter.getCurPageInBook());
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (v == null) {
                    if (DEBUG)
                        Log.d(TAG, "action down content is null so we intercept touch event directly");
                    ...
                    if (!r.contains((int) mMotionX, (int) mMotionY)) {
                        if (DEBUG)
                            Log.d(TAG, "action down touch point not in content so we intercept touch event");
                        if (canDrag(mDownX, mDownY)) {
                            changeMode(DRAG_MODE);
                        }
                    } else {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (distance >= MOTION_SLOT) {
                    if (DEBUG)
                        Log.d(TAG, "action move moved distance > slot so intercept touch event");
                    if (!canDrag(mDownX, mDownY)) {
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (mIsAnimating) {
                mIsAnimating = false;
                doTurnIfNeed(mIsTurnBack, mTurnOrReset);
    ...
    /**
     * mIsTurnBack=true上一页，mIsTurnBack=false并且mTurnOrReset=true下一页
     *
     * @param mIsTurnBack
     * @param mTurnOrReset
     */
    public void doTurnIfNeed(boolean mIsTurnBack, boolean mTurnOrReset) {
        mIsAnimating = false;
        mIsInterActMode = false;
    ...
    public void recycleBitmap() {
        if (mPrePageBitmap != null) {
            mPrePageBitmap.recycle();
            mPrePageBitmap = null;
        }
