...
public class PageContainer extends FrameLayout {
    ...
    void doTurnReload(boolean isTurnBack, int currentPage) {
        if (mIPageContainter == null) {
            if (MagicBookView.DEBUG)
                Log.e(MagicBookView.TAG, "PageContainter IPageContainter is null");
            return;
        } else if (mPageInBook < 0) {
            if (MagicBookView.DEBUG)
                Log.i(MagicBookView.TAG, "PageContainter PageInBook is -1 will not reload");
            return;
        } else if (mPageInBook >= mPageCount) {
            if (MagicBookView.DEBUG)
                Log.i(MagicBookView.TAG, "PageContainter PageInBook >= PageCount will not reload");
            return;
        } else {
